...
// Cargar información del libro al modal de confirmación (eliminar)
document.addEventListener('DOMContentLoaded', function () {
    const confirmModal = document.getElementById('confirmModal');
    if (confirmModal) {
        confirmModal.addEventListener('show.bs.modal', function (event) {
            const button = event.relatedTarget;
            document.getElementById('modalIdLibro').value = button.getAttribute('data-bs-id');
            document.getElementById('modalTituloLibro').textContent = button.getAttribute('data-bs-descripcion');
        });
    }
});
